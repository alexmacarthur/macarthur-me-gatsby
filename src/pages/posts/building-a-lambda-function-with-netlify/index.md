...
Notice that even though we've created a `lambda` directory to contain our compiled function's code, we're _not_ actually targeting it with any of these commands. Instead, `netlify-lambda` will handle that itself. When the `serve` or `build` command is executed, the contents of the target directory are automatically transferred to the directory we'll define in our `netlify.toml` file in the next step.
